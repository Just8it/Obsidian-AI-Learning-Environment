...
${r}
Output ONLY raw cards using the MANDATORY FORMAT above.`}async generateSummary(e){var p,y;let t=this.getContent();if(!t){new c.Notice("No active markdown note.");return}if(t.text.trim().length<50){new c.Notice("\u26A0\uFE0F Content too short.");return}let a=e.model||this.settings.model,n=((p=this.settings.modelContextLengths)==null?void 0:p[a])||128e3,s=Math.max(1e4,Math.floor((n-4e3)*3)),i=[];for(let v=0;v<t.text.length;v+=s)i.push(t.text.substring(v,v+s));new c.Notice(`\u{1F4DD} Generating summary (${i.length} chunks)...`);let r=[],o="",d=this.buildSummaryPrompt(e),h=e.summaryCustomInstructions?`
